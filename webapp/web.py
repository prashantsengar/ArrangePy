...
from webapp import RESULT_DIR, FOLDER_TYPES, TARGET_FOLDER
from webapp import arrange
...
def standardScan():
    ...
    report = arrange.weak_arrange(TARGET_DIR, destination, FOLDER_TYPES)
...
def deepScan():
    ...
    TARGET_DIR = gettheAddress()
    destination = startwork(TARGET_DIR)
    report = arrange.strong_arrange(TARGET_DIR, destination, FOLDER_TYPES)
    others = "Others(Not_moved)"
    if others not in report:
        if len(report) == 0:
            report[others] = "No file to move,"
